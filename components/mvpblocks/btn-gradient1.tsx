...
export default function Btn09({text, link}:{text?: string, link: string}) {
  return (
    <Button className="group relative inline-flex items-center justify-center overflow-hidden rounded-md bg-gray-800 px-8 py-2.5 tracking-tighter text-white">
      ...
      <Link href={link}><span className="relative text-base font-semibold">{text ? text: 'Hover Me'}</span></Link>
    </Button>
  );
}
